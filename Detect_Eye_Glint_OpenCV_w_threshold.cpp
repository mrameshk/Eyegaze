...
int main ( int argc, char **argv )
{
    ...
    //float frame_1_copy, frame_2_copy;
    
    //printf("start Your time\n");
	start_time = clock();
    ...
    for(;;)
	{
		...
		detect(frame_1_copy, frame_2_copy);
...
void imageProcessing(cv::Mat &image1,cv::Mat &image2)
{
		...
		#if DEBUG_FLAG == 1
		imshow( "Video", scaledImage1 ); // preview (only in a debug show ), take image nd display on a window
		imshow( "Video", scaledImage2 );
		#endif
		
        diffImage = abs(scaledImage1-scaledImage2);
...
void detect(cv::Mat &image1,cv::Mat &image2)
{
	...
	#if DEBUG_FLAG == 1
	cv::circle(rotated_img, cvPoint(xpos, ypos),5, CV_RGB(0,0,255), 4, 8,0);
